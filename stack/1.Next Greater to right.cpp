...
/* Question:Find the next greatest element to the right,if there is no greater element to the right,then give output as -1.
...
Here we will traverse the array from right to left.We will store elements that are greater than the elements that come along the way 
as we already know that we have greater value element ahead.
As we are traversing and storing our answer in the reverse order,we will have to reverse the vector and print at the end
